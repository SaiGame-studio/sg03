...
| `ERR_ABILITY_BACK_STAB_NO_GOBLIN` | `back_stab requires untriggered goblin character in front_line` | Backstab requires an active Goblin character on the front line. | Đâm Lén yêu cầu nhân vật Goblin ở hàng trước. | `lib_ability_natureborn.lua` |
| `ERR_ABILITY_BACK_STAB_SELF_TARGET` | `back_stab cannot target the selected goblin` | Backstab cannot target the acting Goblin itself. | Đâm Lén không thể tự nhắm vào bản thân. | `lib_ability_natureborn.lua` |
| `ERR_ABILITY_BRUTE_CALL_NO_SHAMAN` | `brute_call requires a Goblin Shaman target` / `brute_call target must be Goblin Shaman` | Brute Call requires targeting a Goblin Shaman. | Triệu Hồi Đồ Tể yêu cầu Goblin Shaman. | `lib_ability_natureborn.lua` |
...
| `ERR_ABILITY_STATIC_BIND_DAMAGE_INVALID` | `static_bind requires a positive base_stats.stun_damage` | Static Bind requires a positive stun damage value. | Static Bind yêu cầu chỉ số stun_damage dương. | `lib_ability_lightborn.lua` |
...
| `ERR_ABILITY_LIGHTNING_STRIKE_NO_ADJACENT_TARGET` | `lightning_strike requires an adjacent target character` | Lightning Strike requires an adjacent Character target. | Lightning Strike yêu cầu một Character mục tiêu liền kề. | `lib_ability_lightborn.lua` |
...
| `ERR_ABILITY_LIGHTNING_STRIKE_DAMAGE_INVALID` | `lightning_strike requires a positive base_stats.stun_damage` | Lightning Strike requires a positive stun damage value. | Lightning Strike yêu cầu chỉ số stun_damage dương. | `lib_ability_lightborn.lua` |
...
| `ERR_ABILITY_SKELETON_SHIELD_NO_HELLSCYTHE` | `skeleton_shield requires untriggered hellscythe in front_line` | Skeleton Shield requires Hellscythe on the front line. | Khiên Xương yêu cầu Hellscythe ở hàng trước. | `lib_ability_darkborn.lua` |
