...
| `ERR_ABILITY_ANIMATE_DEAD_NO_RIA` | `animate_dead requires untriggered ria in front_line` | Animate Dead requires an active Ria on the front line. | Gọi Hồn yêu cầu Ria ở hàng trước. | `lib_ability_advanced.lua` |
| `ERR_ABILITY_KING_RETURN_NO_RIA_TARGET` | `king_return requires a Ria target` / `king_return target must be Ria` / `king_return target must be on own front_line` / `king_return requires an untriggered Ria in own front_line` | King Return requires targeting an untriggered Ria on your front line. | King Return yêu cầu nhắm Ria chưa kích hoạt ở hàng trước của bạn. | `lib_ability_advanced.lua` |
...
| `ERR_ABILITY_KING_RETURN_NO_SKELETON` | `king_return requires 3 Skeleton in own front_line` | King Return requires three Skeleton cards on your front line. | King Return yêu cầu ba Skeleton ở hàng trước của bạn. | `lib_ability_advanced.lua` |
