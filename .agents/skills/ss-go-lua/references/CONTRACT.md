...
Concrete fields for every table or list result are defined in `RETURN_SCHEMAS.md`. Read that reference before accessing returned fields.
...
| `game.get_item_def_by_id(id)` | `SSItemDefinition, err` | Fetches an item definition by UUID. |
| `game.get_item_def_by_code(code)` | `SSItemDefinition, err` | Fetches an item definition by code. |
| `game.get_item_defs_by_ids(ids)` | `SSItemDefinition[], err` | Fetches multiple item definitions by UUID array — single DB query. |
| `game.get_item_defs_by_codes(codes)` | `SSItemDefinition[], err` | Fetches multiple item definitions by code array — single DB query. |
| `game.get_item_instance_by_id(id)` | `SSInventoryItem, err` | Fetches a player inventory item instance by UUID. |
...
| `game.get_entity_def_by_id(id)` | `SSEntityDefinition, err` | Fetches an entity definition by UUID. |
| `game.get_entity_def_by_key(key)` | `SSEntityDefinition, err` | Fetches an entity definition by key. |
| `game.entity_pool_random(pool_key)` | `SSEntityPoolSelection, err` | Weighted random entity from a pool. |
| `game.entity_pool_min(pool_key, stat_key [, count])` | `SSEntityPoolSelection[], err` | Lowest-stat entities. Always a list; `count` max is 100. |
| `game.entity_pool_max(pool_key, stat_key [, count])` | `SSEntityPoolSelection[], err` | Highest-stat entities. Always a list; `count` max is 100. |
| `game.get_entity_pool_def_by_id(id)` | `SSEntityPool, err` | Fetches an entity pool definition by UUID. |
| `game.get_entity_pool_def_by_key(pool_key)` | `SSEntityPool, err` | Fetches an entity pool definition by key. |
| `game.get_preset_def_by_id(id)` | `SSPresetDefinition, err` | Fetches a preset definition by UUID. |
| `game.get_preset_by_id(id)` | `SSPreset, err` | Fetches a preset instance by UUID. |
| `game.get_preset_slots(preset_id)` | `SSPresetSlot[], err` | Fetches preset slots. |
| `game.get_equipped_in_slot(slot_key)` | `SSInventoryItem, err` | Fetches the player's equipped item in a slot. |
